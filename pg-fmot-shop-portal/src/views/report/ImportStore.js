...
export function ImportStore({show, onHide, updateList}) {
    ...
    const importForAdmin = (file) => {
        ...
        api.couponImportForAdmin(formData).then((res) => {
            console.log("res", res);
            if (res) {
                if (0 === res.data.code) {
                                ...
                                <div className='import-error'>
                                    {
                                        resultTxt.map((value) => (
                                            <div>
                                                ...
                                                <span className='email'>Coupon code：</span>{value.couponCode},
    ...
    const pointImportTemplateUrl = () => {
        ...
        xhh.open("post", '/api/get/pointImportTemplateUrl?type=2',true);
    ...
    return (
        <React.Fragment>
            <Form form={form} layout="vertical">
                <Drawer
                    title="Upload Coupon Payment Status"
                    width={700}
                    visible={show}
                    // maskClosable={false}
                    onClose={() => {
                ...
                >
                    <h2 style={{marginBottom: 10}}><span style={{color: 'red'}}>* </span>Upload Coupon List</h2>
                    <h4 style={{marginBottom: 10}}>
                        Tips: Please upload the .xlsx file with coupon code.
