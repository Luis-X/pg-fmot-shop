    ...
    restructMenu(data, role) {
        return data.map(
            (cv) => {
                ...
                if (cv.childs == null) {
                    return <Menu.Item key={cv.key} onClick={cv.onClick}>
                        {cv.txt}
                    </Menu.Item>
                } else {
                    return <SubMenu key={cv.key} title={cv.txt}>
                        {
    ...
    render() {
        ...
        let dataRight = [
            ...
            },
            {
                ...
                onClick: null,
                // txt: userName,
                txt: <div style={{marginLeft: '-15px'}}>{userName}
                    <span style={{marginLeft: '3px', color: 'orange', fontSize: '14px'}}>
                        ({roleName})
                    </span>
                </div>,
                childs: [
                    ...
                    <Row>
                        <Col span={20}>
                            <Menu
                                ...
                                {/*           onClick={() => {*/}
                                {/*           }}>*/}
                                {/*    <img alt="" style={{width: '60px'}} src={logImg}/>*/}
                                {/*</Menu.Item>*/}
                                {
                        ...
                        <Col span={4}>
                            <Menu mode="horizontal"
                                // theme="dark"
                                  style={{lineHeight: '60px', background: '#0071c0', color: '#fff'}}>
                                <Menu.Item key='log'
                                           style={{border: '0px', paddingRight: '0px', color: '#fff'}}
                                           onClick={() => {
                                           }}>
